...
class DCGAN:
    def __init__(self, img_shape, epochs=50000, lr_gen=0.0001, lr_disc=0.0001, z_shape=100, num_classes = 256, batch_size=100, beta1=0.5, epochs_for_sample=500):
        ...
        filenames, labels = get_image_path_and_labels()
        dataset = make_dataset(filenames, labels, num_classes)
        dataset = dataset.repeat(self.epochs)  # set epoch
    ...
    def train(self):
        ...
        for i in range(self.epochs):
            ...
            if i % self.epochs_for_sample == 0:
                ...
                print(f"Epoch: {i}. ptime: {per_epoch_ptime}. Discriminator loss: {d_loss}. Generator loss: {g_loss}")
        ...
        # Calling plot function for losses
        self.generate_plots(self.losses)
    ...
    def generate_sample(self, epoch):
        ...
        imgs = self.sess.run(self.gen_out, feed_dict={self.phZ:z, self.phY_g:batch_Y_g})
        self.samples.append(imgs)
        saver.save(self.sess, './checkpoints/generator.ckpt')
        imgs = imgs*0.5 + 0.5
        # scale between 0, 1
    ...
    def generate_plots(self, losses):
        fig, ax = plt.subplots()
        self.losses = np.array(self.losses)
        plt.plot(self.losses.T[0], label='Discriminator')
        plt.plot(self.losses.T[1], label='Generator')
        plt.title("Training Losses")
        plt.legend()
        fig.savefig("loss/loss.png")
    ...
    def generate_random_sample(self):
        ...
        for i in range(c):
            for j in range(r):
                ...
                axs[i, j].axis('off')
                cnt += 1
        fig.savefig("generated/generated.png")
        plt.close()
